...
RSpec.describe OrganizationPolicy, type: :policy do
  ...
  permissions :index? do
    it "grants access to super_admin" do
      expect(subject).to permit(super_admin, other_org)
    end
  ...
  permissions :show? do
    it "grants access to super_admin for any org" do
      expect(subject).to permit(super_admin, other_org)
    end
  ...
  describe OrganizationPolicy::Scope do
    it "returns all organizations for super_admin" do
      other_org
      skill_block_org
      scope = described_class.new(super_admin, Organization).resolve
